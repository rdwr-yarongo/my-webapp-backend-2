<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Server Webpage</title>
  <style>
    :root {
      --bg: #d7dee8;
      --card: #ffffff;
      --text: #1f2937;
      --muted: #6b7280;
      --border: #e5e7eb;
      --primary: #d71920;
      --primary-dark: #b31318;
      --warning: #f59e0b;
      --danger: #dc2626;
      --success: #16a34a;
      --shadow: 0 6px 18px rgba(15, 23, 42, 0.06);
      --radius: 12px;
    }

    * {
      box-sizing: border-box;
    }

    body {
      margin: 0;
      font-family: Arial, Helvetica, sans-serif;
      background: linear-gradient(135deg, #eef2f7 0%, var(--bg) 100%);
      color: var(--text);
      min-height: 100vh;
    }

    .page {
      width: min(1100px, 94vw);
      margin: 0 auto;
      padding: 10px 0 28px;
    }

    .header {
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 12px;
      margin-bottom: 10px;
    }

    .header-side {
      display: flex;
      align-items: center;
      gap: 10px;
    }

    .brand {
      display: flex;
      align-items: center;
      gap: 12px;
      min-width: 0;
    }

    .brand-title h1 {
      margin: 0;
      font-size: 22px;
      line-height: 1.1;
    }

    .brand-title p {
      margin: 2px 0 0;
      color: var(--muted);
      font-size: 12px;
    }

    .status-pill {
      display: inline-flex;
      align-items: center;
      gap: 6px;
      padding: 5px 8px;
      border: 1px solid var(--border);
      background: var(--card);
      border-radius: 999px;
      box-shadow: 0 4px 12px rgba(15, 23, 42, 0.05);
      font-size: 11px;
      line-height: 1;
      white-space: nowrap;
    }

    .status-dot {
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background: var(--success);
      box-shadow: 0 0 0 3px rgba(22, 163, 74, 0.1);
    }

    .server-badge {
      width: 78px;
      height: 78px;
      border-radius: 50%;
      border: 2px solid rgba(47, 111, 216, 0.22);
      background: radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.96), rgba(235, 244, 255, 0.88));
      display: inline-flex;
      align-items: center;
      justify-content: center;
      box-shadow: 0 8px 18px rgba(47, 111, 216, 0.08);
      flex-shrink: 0;
    }

    .server-badge img {
      width: 42px;
      height: 42px;
      object-fit: contain;
      display: block;
    }

    .grid {
      display: grid;
      grid-template-columns: 1fr;
      gap: 14px;
      align-items: start;
    }

    .card {
      background: var(--card);
      border: 1px solid var(--border);
      border-radius: var(--radius);
      box-shadow: var(--shadow);
      overflow: hidden;
    }

    .card-header {
      padding: 12px 16px;
      border-bottom: 1px solid var(--border);
      display: flex;
      align-items: center;
      justify-content: space-between;
      gap: 14px;
    }

    .card-header h2 {
      margin: 0;
      font-size: 15px;
    }

    .card-header span {
      color: var(--muted);
      font-size: 12px;
    }

    .info-table {
      width: 100%;
      border-collapse: collapse;
    }

    .info-table tr:not(:last-child) {
      border-bottom: 1px solid var(--border);
    }

    .info-table td {
      padding: 8px 16px;
      vertical-align: top;
      font-size: 13px;
    }

    .info-table td:first-child {
      width: 32%;
      color: var(--muted);
      font-weight: 600;
    }

    .info-value {
      font-family: Consolas, Monaco, monospace;
      word-break: break-word;
    }

    .actions {
      padding: 18px;
      display: grid;
      gap: 12px;
    }

    .action-button {
      width: 100%;
      border: 0;
      border-radius: 12px;
      padding: 14px 16px;
      font-size: 14px;
      font-weight: 700;
      color: #fff;
      cursor: pointer;
      transition: transform 0.15s ease, box-shadow 0.15s ease, opacity 0.15s ease;
      text-align: left;
    }

    .action-button:hover {
      transform: translateY(-1px);
      box-shadow: 0 8px 18px rgba(15, 23, 42, 0.16);
    }

    .action-button:active {
      transform: translateY(0);
      opacity: 0.9;
    }

    .danger { background: var(--danger); }
    .warning { background: var(--warning); }
    .primary { background: var(--primary); }
    .success { background: var(--success); }

    details {
      background: var(--card);
      border: 1px solid var(--border);
      border-radius: var(--radius);
      box-shadow: var(--shadow);
      margin-top: 22px;
      overflow: hidden;
    }

    summary {
      cursor: pointer;
      padding: 18px 22px;
      font-weight: 700;
      list-style: none;
      border-bottom: 1px solid transparent;
    }

    summary::-webkit-details-marker {
      display: none;
    }

    details[open] summary {
      border-bottom-color: var(--border);
    }

    .headers-wrap {
      max-height: 300px;
      overflow: auto;
    }

    .cards-compact {
      display: grid;
      grid-template-columns: 1fr 1fr;
      gap: 14px;
      margin-top: 14px;
    }

    .rewrite-marker {
      margin-top: 14px;
      padding: 16px 18px;
      border: 2px dashed var(--primary);
      border-radius: var(--radius);
      background: #fff5f5;
      box-shadow: var(--shadow);
    }

    .browser-probe {
      margin-top: 14px;
      background: var(--card);
      border: 1px solid var(--border);
      border-radius: var(--radius);
      box-shadow: var(--shadow);
      overflow: hidden;
    }

    .browser-probe summary {
      padding: 14px 16px;
      border-bottom: 1px solid transparent;
    }

    .browser-probe[open] summary {
      border-bottom-color: var(--border);
    }

    .browser-probe-copy {
      padding: 12px 16px 0;
      color: var(--muted);
      font-size: 12px;
    }

    .browser-probe-grid {
      display: grid;
      grid-template-columns: repeat(3, minmax(0, 1fr));
      gap: 14px;
      padding: 14px 16px 16px;
    }

    .rewrite-marker h2 {
      margin: 0 0 6px;
      font-size: 15px;
    }

    .rewrite-marker-code {
      display: inline-block;
      margin-top: 6px;
      padding: 6px 10px;
      border-radius: 10px;
      background: #1f2937;
      color: #ffffff;
      font-family: Consolas, Monaco, monospace;
      font-size: 18px;
      font-weight: 700;
      letter-spacing: 0.06em;
    }

    .message-form {
      padding: 20px 22px;
    }

    textarea {
      width: 100%;
      min-height: 120px;
      resize: vertical;
      border: 1px solid var(--border);
      border-radius: 12px;
      padding: 12px 14px;
      font-family: Arial, Helvetica, sans-serif;
      font-size: 14px;
      outline: none;
    }

    textarea:focus {
      border-color: var(--primary);
      box-shadow: 0 0 0 4px rgba(215, 25, 32, 0.12);
    }

    .submit-button {
      margin-top: 12px;
      border: 0;
      border-radius: 12px;
      padding: 12px 18px;
      background: var(--primary);
      color: #fff;
      font-weight: 700;
      cursor: pointer;
    }

    .footer {
      margin-top: 28px;
      text-align: center;
      color: var(--muted);
      font-size: 13px;
    }

    @media (max-width: 820px) {
      .header,
      .grid,
      .cards-compact,
      .browser-probe-grid {
        grid-template-columns: 1fr;
      }

      .header {
        align-items: flex-start;
        flex-direction: column;
      }

      .brand {
        width: auto;
        justify-content: flex-start;
        align-items: center;
        gap: 10px;
      }

      .header-side {
        width: auto;
        align-self: flex-start;
      }

      .status-pill {
        white-space: normal;
      }

      .brand-title h1 {
        font-size: 18px;
      }

      .brand-title p {
        font-size: 11px;
      }

      .server-badge {
        width: 64px;
        height: 64px;
      }

      .server-badge img {
        width: 36px;
        height: 36px;
      }
    }
  </style>
</head>
<body onload="InitPage();">
  <?php
    function randStr($length = 20) {
      $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
      $string = '';
      for ($i = 0; $i < $length; $i++) {
        $string .= $characters[mt_rand(0, strlen($characters) - 1)];
      }
      return $string;
    }
    if (!isset($_COOKIE['ServerCookie'])) { setcookie("ServerCookie", randStr()); }
  ?>

  <main class="page">
    <header class="header">
      <div class="brand">
        <div class="server-badge" aria-hidden="true">
          <img src="images/servers.png" alt="" />
        </div>
        <div class="brand-title">
          <h1>Server Webpage</h1>
          <p>Server visibility dashboard</p>
        </div>
      </div>
      <div class="header-side">
        <div class="status-pill">
          <span class="status-dot"></span>
          <span>Served by Apache</span>
        </div>
      </div>
    </header>

    <section class="grid">
      <div class="card">
        <div class="card-header">
          <div>
            <h2>Connection Details</h2>
            <span>Live request and server information</span>
          </div>
        </div>

        <table class="info-table">
          <tr>
            <td>Protocol Version</td>
            <td class="info-value" id="ProtocolVer">Detecting...</td>
          </tr>
          <?php
            $headers = array();
            echo '<tr><td>Client IP</td><td class="info-value">' . getenv('REMOTE_ADDR') . '</td></tr>';
            echo '<tr><td>Client Source Port</td><td class="info-value">' . getenv('REMOTE_PORT') . '</td></tr>';

            $host = getenv('HTTP_HOST');
            if (strpos($host, 'scenario3') !== false) {
              if (getenv('SERVER_ADDR') == '10.100.3.11') {
                echo '<tr><td>Server Type</td><td class="info-value">Development</td></tr>';
              } elseif (getenv('SERVER_ADDR') == '10.100.3.12' || getenv('SERVER_ADDR') == '10.100.3.13') {
                echo '<tr><td>Server Type</td><td class="info-value">Staging</td></tr>';
              } else {
                echo '<tr><td>Server Type</td><td class="info-value">Production</td></tr>';
              }
            }

            $srv_ip = getenv('SERVER_ADDR');
            if ($srv_ip == '10.100.3.11') { $srv = "Server1"; }
            elseif ($srv_ip == '10.100.3.12') { $srv = "Server2"; }
            elseif ($srv_ip == '10.100.3.13') { $srv = "Server3"; }
            elseif ($srv_ip == '10.100.3.14') { $srv = "Server4"; }
            elseif ($srv_ip == '10.100.3.15') { $srv = "Server5"; }
            else { $srv = "Server0"; }

            echo '<tr><td>Server</td><td class="info-value">' . $srv . '</td></tr>';
            echo '<tr><td>Server IP</td><td class="info-value">' . getenv('SERVER_ADDR') . '</td></tr>';
            echo '<tr><td>Server Destination Port</td><td class="info-value">' . getenv('SERVER_PORT') . '</td></tr>';

            $ip = getenv('REMOTE_ADDR');
            if (getenv('REMOTE_ADDR') == '10.100.3.150') {
              echo '<tr><td>Served By</td><td class="info-value">Site A - Alteon 1</td></tr>';
            } elseif (getenv('REMOTE_ADDR') == '10.100.3.151') {
              echo '<tr><td>Served By</td><td class="info-value">Site A - Alteon 2</td></tr>';
            }
          ?>
        </table>
      </div>

      
    </section>

    <div class="cards-compact">
    <div class="card">
      <div class="card-header">
        <div>
          <h2>Request Headers Received By Server</h2>
          <span>Headers observed by the application for the current page request</span>
        </div>
      </div>
      <div class="headers-wrap">
        <table class="info-table">
          <?php
            echo '<tr><td>Request URI</td><td class="info-value">' . getenv('REQUEST_URI') . '</td></tr>';
            foreach ($_SERVER as $name => $value) {
              if (strpos($name, 'HTTP_') === 0) {
                $headers = str_replace(' ', '-', ucwords(str_replace('_', ' ', strtolower(substr($name, 5)))));
                if ($headers === 'X-Forwarded-For') { $ip = $value; }
                echo '<tr><td nowrap>' . $headers . '</td><td class="info-value">' . $value . '</td></tr>';
              }
            }
            header("IP: $ip");
          ?>
        </table>
      </div>
    </div>

    <div class="card">
      <div class="card-header">
        <div>
          <h2>Response Headers Generated/Exposed By Server</h2>
          <span>Headers visible to the application/server for the current page request</span>
        </div>
      </div>
      <div class="headers-wrap">
        <table class="info-table">
          <?php
            $responseHeaders = array();
            foreach (headers_list() as $headerLine) {
              $parts = explode(':', $headerLine, 2);
              $name = trim($parts[0]);
              $value = isset($parts[1]) ? trim($parts[1]) : '';
              if ($name !== '') {
                $responseHeaders[$name] = $value;
              }
            }

            if (function_exists('apache_response_headers')) {
              foreach (apache_response_headers() as $name => $value) {
                if ($name !== '') {
                  $responseHeaders[$name] = $value;
                }
              }
            }

            $statusCode = http_response_code();
            $statusTexts = array(
              200 => 'OK',
              201 => 'Created',
              204 => 'No Content',
              301 => 'Moved Permanently',
              302 => 'Found',
              304 => 'Not Modified',
              400 => 'Bad Request',
              401 => 'Unauthorized',
              403 => 'Forbidden',
              404 => 'Not Found',
              500 => 'Internal Server Error',
              502 => 'Bad Gateway',
              503 => 'Service Unavailable'
            );
            $statusText = isset($statusTexts[$statusCode]) ? $statusTexts[$statusCode] : '';
            $responseHeaders = array_merge(
              array('Status' => trim($statusCode . ' ' . $statusText)),
              $responseHeaders
            );

            if (!isset($responseHeaders['Server'])) {
              $responseHeaders['Server'] = getenv('SERVER_SOFTWARE') ?: 'Apache';
            }

            foreach ($responseHeaders as $name => $value) {
              echo '<tr><td>' . htmlspecialchars($name) . '</td><td class="info-value">' . htmlspecialchars($value) . '</td></tr>';
            }
          ?>
        </table>
        <p style="margin:12px 16px 0;color:#6b7280;font-size:12px;">
          This page shows headers visible to the application/server for the current page request.
          Browser devtools may show additional transport headers not exposed to PHP.
        </p>
      </div>
    </div>
    </div>

    <details class="browser-probe">
      <summary>Browser Client Perspective</summary>
      <div class="browser-probe-copy">
        These values reflect a browser probe request made after page load. They show the browser/client perspective for the probe, not the original page navigation.
      </div>
      <div class="browser-probe-grid">
        <div class="card">
          <div class="card-header">
            <div>
              <h2>Browser Perspective - General</h2>
              <span>Browser and backend-observed probe metadata</span>
            </div>
          </div>
          <div class="headers-wrap">
            <table class="info-table" id="browser-general-table">
              <tr><td>Status</td><td class="info-value">Loading...</td></tr>
            </table>
          </div>
        </div>

        <div class="card">
          <div class="card-header">
            <div>
              <h2>Browser Perspective - Request Headers</h2>
              <span>Headers the server received for the browser probe</span>
            </div>
          </div>
          <div class="headers-wrap">
            <table class="info-table" id="browser-request-headers-table">
              <tr><td>Status</td><td class="info-value">Loading...</td></tr>
            </table>
          </div>
        </div>

        <div class="card">
          <div class="card-header">
            <div>
              <h2>Browser Perspective - Response Headers</h2>
              <span>Headers exposed to browser JavaScript for the probe response</span>
            </div>
          </div>
          <div class="headers-wrap">
            <table class="info-table" id="browser-response-headers-table">
              <tr><td>Status</td><td class="info-value">Loading...</td></tr>
            </table>
          </div>
        </div>
      </div>
    </details>

    <section class="rewrite-marker" aria-label="ADC rewrite test marker">
      <h2>ADC Rewrite Test</h2>
      <p>This marker is intentionally static for body-rewrite validation.</p>
      <div class="rewrite-marker-code">ADC_TEST_APACHE</div>
    </section>

    <details>
      <summary>Post a Message</summary>
      <form class="message-form" action="echo.php" method="post">
        <textarea id="Msg" name="Msg" placeholder="Type a message to echo back..."></textarea>
        <br />
        <input class="submit-button" type="submit" name="Go!" value="Submit" />
      </form>
    </details>

    <p class="footer">Served by Apache</p>
  </main>

  <script>
    function setProbeTableRows(tableId, rows, emptyMessage) {
      const table = document.getElementById(tableId);
      if (!table) return;

      table.innerHTML = '';

      if (!rows.length) {
        table.innerHTML = `<tr><td>Status</td><td class="info-value">${emptyMessage}</td></tr>`;
        return;
      }

      rows.forEach(([name, value]) => {
        const row = document.createElement('tr');
        const nameCell = document.createElement('td');
        const valueCell = document.createElement('td');
        nameCell.textContent = name;
        valueCell.className = 'info-value';
        valueCell.textContent = value;
        row.appendChild(nameCell);
        row.appendChild(valueCell);
        table.appendChild(row);
      });
    }

    function ProtocolVer() {
      const protocolElement = document.getElementById("ProtocolVer");
      const navEntry = performance.getEntriesByType("navigation")[0] || performance.getEntries()[0];
      protocolElement.innerHTML = navEntry && navEntry.nextHopProtocol ? navEntry.nextHopProtocol : "Unavailable";
    }

    async function LoadBrowserPerspective() {
      try {
        const probeUrl = `browser_probe.php?ts=${Date.now()}`;
        const response = await fetch(probeUrl, {
          method: 'GET',
          cache: 'no-store',
          credentials: 'same-origin'
        });

        const data = await response.json();
        const navEntry = performance.getEntriesByType("navigation")[0] || performance.getEntries()[0];
        const generalRows = [
          ['Request URL', response.url || data.request_url || probeUrl],
          ['Request Method', data.request_method || 'GET'],
          ['Status Code', `${response.status} ${response.statusText}`.trim()],
          ['Protocol', navEntry && navEntry.nextHopProtocol ? navEntry.nextHopProtocol : 'Unavailable'],
          ['Remote Address (backend view)', data.remote_addr || 'Unavailable'],
          ['Remote Port (backend view)', data.remote_port || 'Unavailable']
        ];

        if (document.referrerPolicy) {
          generalRows.push(['Referrer Policy', document.referrerPolicy]);
        }

        const requestRows = Object.entries(data.request_headers || {});
        const responseRows = [];
        response.headers.forEach((value, name) => {
          responseRows.push([name, value]);
        });

        setProbeTableRows('browser-general-table', generalRows, 'No browser metadata available');
        setProbeTableRows('browser-request-headers-table', requestRows, 'No request headers available');
        setProbeTableRows('browser-response-headers-table', responseRows, 'No response headers available');
      } catch (error) {
        const message = `Probe failed: ${error}`;
        setProbeTableRows('browser-general-table', [], message);
        setProbeTableRows('browser-request-headers-table', [], message);
        setProbeTableRows('browser-response-headers-table', [], message);
      }
    }

    function InitPage() {
      ProtocolVer();
      LoadBrowserPerspective();
    }
  </script>
</body>
</html>
